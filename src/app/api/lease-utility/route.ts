...
// GET /api/lease-utilities -> List all lease utilities
export async function GET() {
  try {
    const leaseUtilities = await prisma.lease_utility.findMany({
      include: {
        utility: true, // Include utility details
        Lease: true,   // Optional: include lease details
      },
    });
    return NextResponse.json({ success: true, data: leaseUtilities });
  } catch (error) {
    console.error(error);
    return NextResponse.json({ success: false, error: "Failed to fetch lease utilities" }, { status: 500 });
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!lease_id || !utility_id) {
      return NextResponse.json({ success: false, error: "lease_id and utility_id are required" }, { status: 400 });
    }

    // Optional: check if the assignment already exists
    const existing = await prisma.lease_utility.findFirst({
      where: { lease_id, utility_id },
    });
    if (existing) {
      return NextResponse.json({ success: false, error: "Utility already assigned to this lease" }, { status: 400 });
    }
